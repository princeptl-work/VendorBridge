...
exports.getInvoice = async (req, res) => {
  try {
    const inv = await Invoice.findById(req.params.id).populate('vendorId').populate('poId', 'poNumber deliveryDate paymentTerms deliveryAddress').populate('createdBy', 'name email');
...
exports.createInvoice = async (req, res) => {
  try {
    ...
    const inv = new Invoice({ invoiceNumber, poId, vendorId: po.vendorId._id, buyerDetails: buyerDetails || { name: process.env.COMPANY_NAME || 'VendorBridge Corp', email: process.env.COMPANY_EMAIL || '' }, items: po.items, subTotal: po.subTotal, taxRate: po.taxRate, taxAmount: po.taxAmount, grandTotal: po.grandTotal, dueDate: dueDate || new Date(Date.now() + 30*24*60*60*1000), paymentTerms: paymentTerms || po.paymentTerms || 'Net 30', notes, status: 'draft', createdBy: req.user._id });
    await inv.save();
    await PurchaseOrder.findByIdAndUpdate(poId, { invoiceGenerated: true });
    await inv.populate('vendorId', 'name email category');
    await ActivityLog.create({ action: 'Invoice Generated', module: 'invoice', entityId: inv._id, entityNumber: inv.invoiceNumber, performedBy: req.user._id, performerName: req.user.name, performerRole: req.user.role, description: `Invoice '${inv.invoiceNumber}' generated from PO '${po.poNumber}' - ₹${inv.grandTotal.toLocaleString()}` });
...
exports.sendInvoice = async (req, res) => {
  try {
    ...
    const inv = await Invoice.findById(req.params.id).populate('vendorId');
    if (!inv) return res.status(404).json({ message: 'Invoice not found' });
    ...
    await inv.save();
    await ActivityLog.create({ action: 'Invoice Sent via Email', module: 'invoice', entityId: inv._id, entityNumber: inv.invoiceNumber, performedBy: req.user._id, performerName: req.user.name, performerRole: req.user.role, description: `Invoice '${inv.invoiceNumber}' sent to ${recipientEmail}` });
...
exports.markPaid = async (req, res) => {
  try {
    ...
    await inv.save();
    await ActivityLog.create({ action: 'Invoice Marked Paid', module: 'invoice', entityId: inv._id, entityNumber: inv.invoiceNumber, performedBy: req.user._id, performerName: req.user.name, performerRole: req.user.role, description: `Invoice '${inv.invoiceNumber}' marked paid${paymentReference ? ` (Ref: ${paymentReference})` : ''}` });
    res.json({ success: true, invoice: inv });
  } catch (e) { res.status(500).json({ message: e.message }); }
};
