...
const InvoiceList = () => {
  ...
  const handleGenerateInvoice = async (po) => {
    ...
    setGenerating(true);
    try {
      const res = await api.post('/invoices', { poId: po._id });
            ...
            {pendingPOs.map(po => (
              <div key={po._id} className="card" style={{ padding: 16, background: '#fff', border: '1px solid var(--primary-border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <button className="btn btn-primary btn-sm" disabled={generating} onClick={() => handleGenerateInvoice(po)}>
                    {generating && selectedPO?._id === po._id ? 'Generating...' : 'Generate Invoice'}
                  </button>
                </div>
              </div>
            ))}
